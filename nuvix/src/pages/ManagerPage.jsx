...
export default function ManagerPage() {
  ...
  const [activeTab, setActiveTab] = useState("overview"); // "overview" | "orders" | "products" | "pricing" | "inventory" | "notifications" | "styles" | "reviews" | "inquiries" | "settings"
  ...
  const openEditProduct = (product) => {
    ...
    setProductForm({
      ...
      gsms: resolvedGsms,
      colors: product.colors && product.colors.length > 0 ? product.colors : (product.fabricColor ? [product.fabricColor] : ["#ffffff"]),
      fabricColor: product.fabricColor || product.colors?.[0] || "#ffffff",
      images: product.images || [],
      modelPath: product.modelPath || "/images/models/male normal t-shirt1.glb",
      defaultColor: product.fabricColor || product.colors?.[0] || "#ffffff",
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-3 space-y-1.5 min-h-0 [scrollbar-width:thin] [scrollbar-color:#334155_transparent]">
          ...
          >
            <span className="flex items-center gap-3 min-w-0">
              ...
              <span className="truncate">Live Notifications</span>
            </span>
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <span className="text-[10px] font-bold text-indigo-600 uppercase tracking-wider block truncate">
                ...
                {activeTab === "notifications" && "Live Notification Center"}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
                  {pendingDrafts.map((draft) => (
                    ...
                    >
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex items-center gap-1.5 min-w-0">
                            <span className="px-2.5 py-1 bg-purple-50 text-purple-700 border border-purple-100 rounded-lg text-[10px] font-black uppercase tracking-wider truncate">
                              {draft.category || "T-Shirts"}
                            </span>
                            <span className="px-2 py-0.5 bg-amber-50 text-amber-700 border border-amber-200/70 rounded-md text-[10px] font-bold shrink-0">
                              Pending
                            </span>
                          </div>
                          <span className="text-xs font-black text-slate-900 bg-slate-100 px-2.5 py-1 rounded-lg shrink-0">
                            Rs. {(draft.basePrice || 0).toFixed(2)}
                          </span>
                        </div>

                        {/* 3D Frozen T-Shirt Card Preview like in Store */}
                        <div className="relative rounded-2xl overflow-hidden shadow-xs border border-slate-100 bg-slate-50">
                          <Store3DCardPreview
                            product={draft}
                            activeColor={draft.fabricColor || draft.colors?.[0] || "#ffffff"}
                            onClick={() => {
                              setSelectedSubmissionProduct(draft);
                              setSubmissionSide("front");
                              setSubmissionZoom(0.85);
                        ...
                        <div className="space-y-1.5">
                          ...
                          {draft.description ? (
                            ...
                            <p className="text-xs text-slate-400 italic line-clamp-2 min-h-[32px]">
                            ...
                            {draft.colors && draft.colors.length > 0 && (
                              <span className="text-[10px] text-slate-500 font-semibold bg-slate-100 px-2 py-0.5 rounded-md flex items-center gap-1">
                                <span
                                  ...
                                  style={{ backgroundColor: draft.fabricColor || draft.colors[0] }}
                                />
                                {draft.colors.length} {draft.colors.length === 1 ? "Color" : "Colors"}
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      {/* CLEAR APPROVE, DISAPPROVE & EDIT BUTTONS */}
                      <div className="pt-3 border-t border-slate-100 space-y-2">
                        <div className="grid grid-cols-2 gap-2">
                          <button
                            type="button"
                            onClick={() => openEditProduct(draft)}
                            className="py-2 px-3 bg-slate-50 hover:bg-indigo-50 text-slate-700 hover:text-indigo-600 border border-slate-200 hover:border-indigo-200 rounded-xl text-xs font-bold transition flex items-center justify-center gap-1.5 cursor-pointer shadow-2xs active:scale-98"
                          ...
                          <button
                            ...
                            className="py-2 px-3 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 rounded-xl text-xs font-bold transition shadow-2xs flex items-center justify-center gap-1.5 disabled:opacity-50 cursor-pointer active:scale-98"
                        ...
                        <button
                          ...
                          className="w-full py-2.5 px-3 bg-emerald-600 hover:bg-emerald-700 active:bg-emerald-800 text-white rounded-xl text-xs font-bold transition shadow-sm hover:shadow-md flex items-center justify-center gap-1.5 disabled:opacity-50 cursor-pointer active:scale-98"
        ...
        {activeTab === "notifications" && (() => {
          ...
          return (
            <div className="bg-white border rounded-3xl p-6 shadow-sm flex flex-col gap-6">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 select-none pb-4 border-b border-slate-100">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">
                      Monitor real-time updates across customer orders, inventory stock levels, and manager production pipelines.
                    </p>
                  </div>
                </div>
              ...
              {/* Notification Type Filters & Sub-filters Bar */}
            ...
            <div className="w-full md:w-[400px] flex flex-col justify-between p-6 bg-white overflow-y-auto">
              ...
              <div className="mt-8 pt-4 border-t flex flex-col gap-2.5">
                <button
                  type="button"
                  onClick={() => {
                    const prodToEdit = selectedSubmissionProduct;
                    setSelectedSubmissionProduct(null);
                    openEditProduct(prodToEdit);
                  }}
                  className="w-full py-3 px-4 bg-slate-100 hover:bg-indigo-50 text-slate-700 hover:text-indigo-600 border border-slate-200 hover:border-indigo-200 rounded-2xl font-bold text-xs shadow-2xs transition flex items-center justify-center gap-2 cursor-pointer"
                  title="Edit Product Details"
                >
