...
export default function StorePage() {
  ...
  const handleProductReviewSubmit = async (e) => {
    ...
    if (isManagerPreview || isManagerOrAdmin) {
      setPreviewNotice({
        ...
        message: "Review submission is disabled in Manager Store Preview. This view is for demonstrating the customer storefront. Live reviews can be managed and moderated in the Manager Dashboard.",
  ...
  const getModalModelPath = () => {
    ...
    if (selected3DProduct.modelPath && typeof selected3DProduct.modelPath === "string" && (selected3DProduct.modelPath.toLowerCase().endsWith(".glb") || selected3DProduct.modelPath.toLowerCase().endsWith(".gltf") || selected3DProduct.modelPath.toLowerCase().endsWith(".fbx"))) {
      return selected3DProduct.modelPath;
    }
    if (selected3DProduct.modelUrl && typeof selected3DProduct.modelUrl === "string" && (selected3DProduct.modelUrl.toLowerCase().endsWith(".glb") || selected3DProduct.modelUrl.toLowerCase().endsWith(".gltf") || selected3DProduct.modelUrl.toLowerCase().endsWith(".fbx"))) {
    ...
    >
          ...
          <div className="max-w-7xl mx-auto space-y-8">
            {isPreviewMode && (
              <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white px-5 py-3 rounded-2xl border border-indigo-500/30 flex items-center justify-between shadow-md">
                ...
                <span className="px-2.5 py-1 bg-indigo-600/40 text-indigo-200 border border-indigo-400/30 rounded-xl text-[10px] font-black uppercase tracking-wider">
                  Preview Active
                </span>
              </div>
            )}
      ...
      {selected3DProduct && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-4xl border shadow-2xl overflow-hidden flex flex-col md:flex-row h-[90vh] md:h-[600px] select-none relative">
      ...
      {/* Manager Store Preview Mode Notice Modal */}
      {previewNotice && (
        <div className="fixed inset-0 z-[100] bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-150 select-none">
          <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl border border-indigo-100 overflow-hidden animate-in zoom-in-95 duration-150">
            {/* Modal Header */}
            <div className="bg-gradient-to-r from-indigo-900 via-indigo-800 to-purple-900 text-white p-6 flex items-start justify-between">
              <div className="flex items-center gap-3">
                <div className="p-3 bg-white/10 backdrop-blur-xs border border-white/20 rounded-2xl text-indigo-200 shadow-inner">
                  {previewNotice.type === "cart" ? (
                ...
                <div>
                  <span className="px-2.5 py-0.5 bg-indigo-500/30 border border-indigo-400/30 text-indigo-200 text-[10px] font-black uppercase tracking-wider rounded-full">
                    Manager Preview
                  </span>
              ...
              <button
                ...
                className="p-1 rounded-xl text-white/60 hover:text-white hover:bg-white/10 transition cursor-pointer"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-4">
              <div className="p-4 rounded-2xl bg-indigo-50/60 border border-indigo-100/80 space-y-1.5">
                ...
                {previewNotice.productTitle && (
                  <p className="text-[11px] text-indigo-600 font-semibold pt-1">
                    Design: <strong>{previewNotice.productTitle}</strong>
                  </p>
                )}
              </div>

              <div className="p-3 bg-amber-50 rounded-xl border border-amber-200/80 text-[11px] text-amber-900 flex items-center gap-2 font-medium">
                <AlertCircle className="h-4 w-4 shrink-0 text-amber-600" />
                <span>This storefront is currently in preview mode for manager demonstration and visual inspection.</span>
              </div>
            </div>
            ...
            <div className="p-4 bg-slate-50 border-t flex items-center justify-end">
              <button
                type="button"
                onClick={() => setPreviewNotice(null)}
                className="w-full sm:w-auto px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold transition shadow-sm cursor-pointer"
              >
